...
def run_pairs_backtest():
    print("=== Starting Pairs Trading Backtest (100d Window - TP -0.5) ===")
    if not BOT_TOKEN or not CHANNEL_ID:
        print("Missing BOT_TOKEN or CHANNEL_ID.")
        return

    total_signals = 0
    total_wins = 0    # Reached Z = -0.5
        ...
        try:
            ...
            for i in range(250, len(data)):
                ...
                if not in_trade:
                    ...
                    # Waiting for Z to cross the mean and hit -0.5 (or +0.5 for shorts)
                    if (trade_direction == 1 and current_z <= -0.5) or (trade_direction == -1 and current_z >= 0.5):
                        total_wins += 1
                        in_trade = False
                        print(f"  -> WIN on {data.index[i].date()} (Z hit -0.5 / +0.5)")
                    # =========================================================
...
⚙️ <b>Logic:</b> Z-Score Entry (>1.0) | TP (Z=-0.5) | SL (Z=4) | Window (100d)

━━━━━━━━━━━━━━━━━━━━
📊 <b>Total Trades Triggered:</b> {total_signals}

🟢 <b>Winners (Overshot Mean):</b> {total_wins}
