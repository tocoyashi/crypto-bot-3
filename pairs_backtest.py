...
def run_pairs_backtest():
    ...
    for yf_ticker1, yf_ticker2, name1, name2 in PAIRS:
        ...
        try:
            ...
            trade_spread_mean = 0
            trade_spread_std = 0
            trade_direction = 0 # 1 for Z > 2, -1 for Z < -2
            ...
            for i in range(250, len(data)):
                ...
                if not in_trade:
                    # --- SIGNAL GENERATION LOGIC (Exactly as in bot.py) ---
                    ...
                    if p_value <= 0.15:
                        ...
                        if not pd.isna(current_z) and spread_std > 1e-8 and abs(current_z) > 2.0:
                            # OPEN TRADE
                            in_trade = True
                            trade_hedge_ratio = hedge_ratio
                            trade_spread_mean = spread_mean
                            trade_spread_std = spread_std
                            trade_direction = 1 if current_z > 2.0 else -1
                            total_signals += 1
                    ...
                    # Check Loss Condition (Stop Loss / Z = 3)
                    elif (trade_direction == 1 and current_z >= 3.0) or (trade_direction == -1 and current_z <= -3.0):
                        total_losses += 1
                        in_trade = False
                        print(f"  -> LOSS on {data.index[i].date()} (Z hit 3)")
...
⚙️ <b>Logic:</b> Z-Score Entry (>2.0) | TP (Z=0) | SL (Z=3)
